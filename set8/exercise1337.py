...
def one_counter(input_list=[1, 4, 1, 5, 1, 1]) -> int:
    ...
    length = len(
        input_list
    )  ## i know there is a shorter way but it was glitching for me

    for i in range(length):
        if input_list[i] is 1:
            count += 1
...
def n_counter(search_for_this, input_list=[1, 4, 1, 5, 1, 1]) -> int:
    ...
    for i in range(length):
        if input_list[i] is search_for_this:
            count += 1
...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    string = str(input_string).upper()

    string = list(string)

    flame = "🔥"

    string = flame.join(string)
...
def best_letter_for_pets() -> str:
    ...
    for letters in the_alphabet:
        pet_containing = pet_filter(letters)
        count = len(pet_containing)
        if count > heighest:
            heighest = count
...
def random_filler_text(number_of_words=200) -> str:
    ...
    for i in range(number_of_words):
        word_add = my_dict[random.randint(3, 7)][random.randint(0, 3)]
